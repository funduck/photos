...
## Layout
- `docker-compose.yml` — all services: `immich-server`, `immich-machine-learning`, `redis`, `database`, `syncthing` (container-side, syncs phone temp folder → `$STORAGE_DIR/Photos`), `rclone-s3-sync` (scheduled S3 backup, see below).
- `docker/rclone-s3-sync/` — custom-built image (Python + rclone) for the scheduled S3 sync. `sync.py` is a loop, not cron: it persists the last-sync timestamp to `/state/<SYNC_NAME>/last_sync.txt` (named volume `rclone_sync_state`) and syncs immediately if the configured `SYNC_INTERVAL` has elapsed — including catch-up after the container was stopped past a scheduled run. Defaults to dry-run. The synced folder is a parameter, not hardcoded: `SYNC_NAME`/`RCLONE_SYNC_SUBDIR` (compose var, default `Photos`) names it on both the host volume mount and the S3 destination subpath — the container always reads from a fixed `/data` internally. To sync a second folder, duplicate the `rclone-s3-sync` service block with a different `container_name`, `RCLONE_SYNC_SUBDIR`, and a separate state volume.
- `scripts/` — one-off/manual shell scripts and a Python dedup tool:
  ...
  - `backup_immich.sh` — stops Postgres, backs up Immich assets + DB to `$STORAGE_DIR/Immich`, restarts. Manual, not scheduled.
  - `duplicate_resolver.py` — calls Immich API to find/delete duplicate assets. `--dry-run`/`--execute`.
  - `com.user.backup_photos.plist` — dead launchd leftover from the pre-syncthing-container pipeline; not active.
...
- Root `.env` (from `.env.example`) — read by `docker-compose.yml` (via compose's auto-loaded root `.env`). Has Immich vars (`DB_PASSWORD`, `IMMICH_DEDUP_API_KEY`, `IMMICH_DIR`, `STORAGE_DIR`) plus, for `rclone-s3-sync`, `RCLONE_REMOTE`, `RCLONE_ACCESS_KEY`, `RCLONE_SECRET_ACCESS_KEY`, `AWS_BUCKET`, `AWS_REGION`, and optional `RCLONE_SYNC_INTERVAL` / `RCLONE_SYNC_DRY_RUN` overrides.
