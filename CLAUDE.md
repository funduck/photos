...
## Layout
- `docker-compose.yml` — all services: `immich-server`, `immich-machine-learning`, `redis`, `database`, `syncthing` (container-side, syncs phone temp folder → `$STORAGE_DIR/Photos`), `rclone-s3-sync` (scheduled S3 backup, see below), `caddy` (HTTPS reverse proxy in front of `immich-server`, see below).
